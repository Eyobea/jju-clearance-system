...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id']) && isset($_POST['delete'])) {
    ...
    $delete_sql = "DELETE FROM library WHERE id = ? LIMIT 1";
...
<head>
    ...
    <title>Liberary Material Records</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <h2>Liberary Material Records</h2>
    <table>
        <tr>
            <th>Student Name</th>
            <th>Student ID</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $id = $row["id"];
                $up = "UPDATE studentclearance SET library=0 WHERE stud_id='$id'";
