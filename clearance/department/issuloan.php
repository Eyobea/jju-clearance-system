...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id']) && isset($_POST['delete'])) {
    ...
    $delete_sql = "DELETE FROM departmente WHERE id = ? LIMIT 1";
...
<head>
    ...
    <title>Sport Material Records</title>
    <style>
        table {
            width: 100%;
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $id = $row["id"];
                $up = "UPDATE studentclearance SET department=0 WHERE stud_id='$id'";
