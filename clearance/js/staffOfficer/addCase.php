<!DOCTYPE HTML"><html>
 <head>
 <?php 
	include_once('header.php');	 
 ?>
 
 </head>
 <body>
  	<table id="navigation" align="center" style="width:device-width;">
	<tr>
		<td class="time">
			<font  color="white" style="font-family:times new roman">
				<?php
					$Today=date('y:m:d');
					$new=date('l, F d, Y',strtotime($Today));
					echo $new;
				?>
			</font> 
		</td>
		<td class="nav">
			<a href="manage.php"><b>..:: register Loan ::..</b></a>
		</td>
		<td class="lio">
			<ul>
				<?php
					session_start();
					if(isset($_SESSION["username"]))
					{
						$userNameD = ucfirst($_SESSION["username"]);
				?>
				<li>
					<?php
						$userNameD = ucfirst($_SESSION["username"]);
						$sql="select * from role where username='$userNameD'";
						$qr=mysql_query($sql);
						while($row = mysql_fetch_array($qr))
						{
							$sfid=$row['staffId'];
							$fn=$row['fullName'];
							$role=$row['role'];
							$camp=$row['campus'];
						}
						$sqls="select * from staff where staffId='$sfid'";
						$qrs=mysql_query($sqls);
						while($row = mysql_fetch_array($qrs))
						{
							$sfid=$row['staffId'];
							$fn=$row['Fname'];
							$mn=$row['Mname'];						
						}
						echo "<a href='$role.php'><div id='un' class='icon-large icon-user'> ".$fn.' '.$mn. "</div></a>";
					?>
				</li> 
				<li class="lo"><a href="../index.php?logged=out">Logout</a></li> 
				<?php
				}
				else
				{ 
					?> 
						<li class="lis"><a href="#login">Login</a></li>               
					<?php
				}
				?>
					
			</ul>
		</td>
		</tr>
	</table>
	
<table id="content" align="center" style="width:device-width;"
	<tr>
		<td class="sidebar">
			<div class="menu">
				<font>Menu</font>
				</div>
<div class="menuList">
				<a href="officer.php"><span class="icon-large icon-home"></span> &nbsp;Home</a></br/>
				<a href="manageItem.php"><span class="icon-large icon-tags"></span> &nbsp;Manage Item</a><br/-->				
				<a href="issuloan.php"><span class="icon-large icon-tags"></span> &nbsp;view issue loan</a><br/-->
				<a href="addcase.php"><span class="icon-large icon-save"></span> &nbsp;add issue loan</a><br/-->
				<a href="viewArchive.php"><span class="icon-large icon-archive"></span> &nbsp;View Archive File</a><br/>
				<a href="http//:www.bdu.edu.et"><span class="icon-large icon-globe"></span> &nbsp;BDU Site </a></br/>
			</div>
		</td>
		<td class="login_area">
			<table width="100%">
				<tr>
				<td class="userl">
					<p><?php echo $role ?> Online Clearance Information System</p>		
				</td>
				</tr>
			</table>			
			<div class="regform">
				<table   cellpadding='4'  cellspacing="0" style="margin-left:25%;width:50%;border-radius:10px; border:5px solid rgba(0,0,0,0.4); box-shadow:0 0 18px rgba(0,0,0,0.4);">
			
				
				<tr  bgcolor="lightblue"><th border="1" colspan="3" style="bgcolor:lightblue"><font>serial No : <?php echo $matid;?><font><a href="issuloan.php"><span style="color:red;margin-left:60%;" class="icon-large icon-remove" align="right" title="close"></span></a></th></tr>
				<tr>
				<td colspan="2">

				<hr/>
				</td></tr>
				<form action="addCase.php" method="post">
				<tr><td><font color="red">*</font><label>staff Id</label></td>
				<td><input type="text" name="id" required pattern="[A-Za-z0-9]+" /></td></tr>
				<tr><td><font color="red">*</font><label>material Id</label></td>
				<td><input type="text" name="mid" required pattern="[A-Za-z0-9]+" /></td></tr>
				
				<tr><td><font color="red">*</font><label>quantity</label> </td>
				<td><input type="number" name="qnt" pattern="[0-9]+" required /><br/><br/></td></tr>	
				<tr><td>
				</td>
				<td>
				<input type="submit" name="addloan" value="register"/>
				<input type="reset" value="Reset"/>
				</td>
		</tr>
		</form>
				<br><br> 

<?php
		if(isset($_POST['addloan']))
								{
						$qn=$_POST['qnt'];
						$idd=$_POST['id'];
						$matid=$_POST['mid'];
						$sql=mysql_query("INSERT INTO `staffcase`(`staffId`, `MaterialId`, `quantity`, `procurmentOffice`, `by_user`, `date_added`, `campus`) VALUES ('$idd','$matid','$qn','$role','$fn','$news','$camp')");
							if($camp=='poly')
							$sql2=  mysql_query("UPDATE polyclearance SET `$role`='1' WHERE staff_id='$idd'");
						else
							if($camp=='peda')
							$sql2=  mysql_query("UPDATE pedaclearance SET `$role`='1' WHERE staff_id='$idd'");
						else
							if($camp=='fb')
							$sql2=  mysql_query("UPDATE fbclearance SET `$role`='1' WHERE staff_id='$idd'");
						else
							if($camp=='yibab')
							$sql2=  mysql_query("UPDATE yibabclearance SET `$role`='1' WHERE staff_id='$idd'");
						else
							if($camp=='zenzelima')
							$sql2=  mysql_query("UPDATE yibabclearance SET `$role`='1' WHERE staff_id='$idd'");
						else
							if($camp=='um')
							$sql2=  mysql_query("UPDATE universitymgmt SET `$role`='1' WHERE staff_id='$idd'");
						
							
						if($sql)
						{
							 
							echo"<tr><td colspan=2>	<div class='alert alert-warning' style='width:50%; float: left; margin-left: 40%;'>
								Sucessfully registred
							</div></td></tr>";
						}							
					}
			
															
			?>
			
	
</table>

</div>
</td>
</tr>
    </table>
    </table>
	<script>
	$("#mydata").dataTable();
	</script>
	<table id="footer" align="center" style="width:device-width;">
	<tr >
		<td><p class="f">Copyright &copy; reserved 2019-BDU-CMS</p></td>
	</tr>
	</table>
 </body>
</html>