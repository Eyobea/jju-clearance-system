  ...
  include 'header.php';
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($idno) || empty($favoriteFood) || empty($primarySchool) || empty($bornPlace) || empty($newPassword)) {
            ...
            $sql = "SELECT * FROM role WHERE Idno = '$idno' AND favorite_food = '$favoriteFood' AND primary_school = '$primarySchool' AND born_place = '$bornPlace'";
            $result = mysqli_query($connection, $sql);
            if (mysqli_num_rows($result) === 1) {
                // Update the password without hashing
                $updateSql = "UPDATE role SET password = '$newPassword' WHERE Idno = '$idno'";
